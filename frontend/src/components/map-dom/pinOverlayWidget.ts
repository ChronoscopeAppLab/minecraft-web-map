...
export default class PinOverlayWidget extends DrawingComponent {
  ...
  showAt(x: number, y: number) {
    if (this.getIsVisible()) {
      ...
      new Animator(300, (ratio: number) => {
        this.setScale(ratio);
      })
        .setDelay(constants.MOVE_ANIMATION_DURATION)
        .setAnimationInterpolator(AnimationInterpolator.overshootInterpolator)
        .start();
    }
  }

  hide() {
    new Animator(100, (ratio: number) => {
      this.setScale(ratio);
    })
      .reversed()
      .withEndAction(() => {
  ...
  move(ratio: number) {
    if (ratio < 0.1) {
      ...
      this.y = this.fromY + (this.toY - this.fromY) * trueratio - (200 * AnimationInterpolator.swanDiveInterpolator(trueratio)) / this.scale;
    } else {
  ...
  draw(dc: DrawingContext) {
    ...
    ctxt.lineTo(20 * Math.sin(Math.PI / 3) * this.scale, -(40 - 20 * Math.cos(Math.PI / 3)) * this.scale);
    ctxt.lineTo(-20 * Math.sin(Math.PI / 3) * this.scale, -(40 - 20 * Math.cos(Math.PI / 3)) * this.scale);
    ctxt.fill();
